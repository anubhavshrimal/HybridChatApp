angular.module('app.services', [])
  ...
  // returns the object of the mobile number specified
  ...
  // returns the object of conversations the user has with a specified number
  .service('conversations', ["rootRef", "$firebaseObject", function (rootRef, $firebaseObject) {
    var usersRef = rootRef.child('conversations');
    this.get = function (chatId) {
      return $firebaseObject(usersRef.child(chatId));
    }
  }])
  ...
  .service("getNumber", ["setNumber", "$localForage", function (setNumber, $localForage) {
    ...
    this.promise = $localForage.getItem("mobileNum").then(function (data) {
      return this.number =  data;
    });
  }])

  // returns the OTP reference object of the specified number
  .service('otpService', ["rootRef", "$firebaseObject", function (rootRef, $firebaseObject) {
    var usersRef = rootRef.child('userInfo');
    this.get = function (mobileNum) {
      return $firebaseObject(usersRef.child(mobileNum).child('OTP'));
    }
  }])

  // set the mobile number in this service
  .service("setNumber", function () {
    this.number;
  })

  // generates a new OTP for the specified number
  .service("OTP", ["users", function (users) {
    this.generate = function (mobileNum) {
      var obj = users.get(mobileNum);
      var OTP = "";
  ...
  // list of all countries name and dial_code
  .factory("dialCodes",function () {
